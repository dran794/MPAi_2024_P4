...
export function updateAnnotations(plotElement, lang) {
  ...
  layout.xaxis.title =
    lang === "en" ? "Tongue Position (F2)" : "Takoto o te arero (F2)";
  layout.yaxis.title =
    lang === "en" ? "Mouth Openness (F1)" : "Tuwhera o te waha (F1)";
...
export function updateFormantEllipses(plotElement, formants, highlightedVowel) {
  const shapes = formants.flatMap((f) =>
    createFormantShape(
      f,
      highlightedVowel ? f.vowel == highlightedVowel : false
    )
  );
  traces = [
    {
...
export function updateAnalysers(analyserElement) {
  ...
  rafID = window.requestAnimationFrame(
    updateAnalysers.bind(this, analyserElement)
  );
}
...
async function doneEncoding(blob, post = true) {
  ...
  window.ksvF0({ noInitialRun: true }).then(async function (Module) {
    ...
    var pitch_results = parse_F0(
      Module.FS.readFile("1.f0", { encoding: "utf8" })
    );
    window.forest({ noInitialRun: true }).then(async function (Module) {
      ...
      Module.callMain(args);
      var results = parse_FMS(
        Module.FS.readFile("1.fms", { encoding: "utf8" })
      );
      console.log(results);
      ...
      results = results.filter(
        (r) => r["F0(Hz)"] > 0 && r["F1(Hz)"] > 0 && r["F2(Hz)"] > 0
      );
      if (results.length == 0) {
      ...
      var keys = Object.keys(results[0]).filter(
        (k) => k.startsWith("F1") == true || k.startsWith("F2") == true
      );
      console.log(keys);
      var coloursTimeline = [];
      ...
      if (timelineElement) {
        ...
        timelineElement.addEventListener("plotly_hover", function (data) {
          ...
          new_trace.marker.line.width = new_trace.x.map((x, i) =>
            i == index ? 2 : 0
          );
          Plotly.react(scatterplotElement, traces, layout);
        });
