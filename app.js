...
const appRoutes = [
    ...
    { name: 'activity-one', path: '/activity-one', component: () => import("./pages/Activity1Page.js")},
    { name: 'activity-two', path: '/activity-two', component: () => import("./pages/Activity2Page.js")},
...
if (urlParams.has("participant_id")) {
  console.log(
    `Participant ID ${urlParams.get(
      "participant_id"
    )} and password in URL, saving.`
  );
...
if (urlParams.has("attemptsAllowed")) {
  ...
  console.log(
    `Setting attempts allowed for recording to ${config.attemptsAllowed}`
  );
}
...
resources.modelSpeakerOptions = await (
  await fetch("samples/samples.json")
).json();
